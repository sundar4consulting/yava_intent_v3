...
2. **ElasticsearchVectorStore** - Production-ready keyword search backend
3. **Persistent Knowledge Base** - Intents stored in Elasticsearch cluster
4. **Keyword Search (BM25)** - Multi-match query with field boosting
5. **Scalable Architecture** - Handles millions of documents vs thousands
...
ARCHITECTURE:
============
Elasticsearch index → Keyword Search (BM25) → RAG classification → LLM enhancement (optional)

DEPENDENCIES:
============
- elasticsearch (pip install elasticsearch)
- openai (pip install openai)
- numpy (pip install numpy) - Optional, only for fallback embedding
...
4. Index uses BM25 algorithm for keyword-based relevance scoring

SEARCH METHOD:
=============
- Uses multi_match query with field boosting
- training_utterances^3, keywords^2, description_short^1.5
- Fuzzy matching (AUTO) for typo tolerance
- Phrase matching boost for exact matches
- BM25 relevance scoring (no embeddings required for search)
...
PERFORMANCE:
===========
- Document Count: Unlimited (millions supported)
- Search Time: 5-15ms (Elasticsearch BM25 keyword search)
- Memory Usage: ~500 KB (ES external)
- Concurrent Users: High (ES cluster scales horizontally)
- No embedding overhead: Direct text matching
"""
...
class ElasticsearchVectorStore:
    ...
    def create_index(self, schema_file: Optional[str] = None):
        """
        Create index with proper mapping for keyword-based search.
        Optimized for BM25 ranking algorithm.
        ...
        if schema_file and os.path.exists(schema_file):
            ...
            # Default schema optimized for keyword search (BM25)
            schema = {
                "settings": {
                    "number_of_shards": 3,
        ...
        try:
            ...
            print(f"✅ Created index: {self.index_name} (keyword search optimized)")
        except Exception as e:
