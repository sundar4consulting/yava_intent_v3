"""
Watson Orchestrate Skill Interface V3 - 3-Key KNN Embeddings + LLM Classifier

NEW IN V3:
==========
- 3-Key KNN embeddings search (intent, description, example)
- Cosine similarity for semantic matching (10-20ms)
...
- All V2 features (Elasticsearch, GPT-4o LLM, slots, multi-intent)

ARCHITECTURE:
=============
V1: InMemoryVectorStore (< 10K vectors)
V2: Elasticsearch BM25 keyword search (millions of documents)
V3: Elasticsearch 3-Key KNN embeddings + BM25 fallback (production-ready)
...
- SENTENCE_TRANSFORMER_MODEL: Model path (default: all-MiniLM-L6-v2)

USAGE:
======
Same API as skill.py and skill_v2.py but uses classifier_v3 backend with KNN.
"""
...
def classify_intent(user_input: str, 
                   ...
                   use_knn: bool = True) -> Dict:
    """
    Tool: Full NLU Classification with Elasticsearch 3-Key KNN + OpenAI LLM Enhancement.
    
    V3 ENHANCEMENTS:
        - 3-Key KNN vector search (intent, description, example embeddings)
        - Cosine similarity semantic matching (10-20ms)
        - Sentence Transformers for query encoding
        - Configurable boosting for embedding keys
        - Automatic fallback to BM25 keyword search
        - All V2 features (LLM invocation, cost tracking, performance metrics)
    ...
    classifier = get_hybrid_classifier_v3(
        ...
        enable_knn=os.getenv("ENABLE_KNN", "true").lower() == "true"
    )
    
    # V3 hybrid classification with KNN
